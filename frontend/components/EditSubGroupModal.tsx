...
interface EditSubGroupModalProps {
  ...
  groups: Group[]; // <-- Recibe las categorías
  onClose: () => void;
  onSave: (data: { id: string; code: string; name: string; description?: string; active: boolean; category_id: string }) => Promise<void> | void;
  saving?: boolean;
}

export const EditSubGroupModal: React.FC<EditSubGroupModalProps> = ({
  subGroup,
  ...
  useEffect(() => {
    if (subGroup) {
      setCode(subGroup.code || '');
      setName(subGroup.name || '');
      setGroupId(subGroup.group_id || ''); // <-- Usar category_id
      setDescription(subGroup.description || '');
      setActive(!!subGroup.active);
      setLocalError(null);
    }
  }, [subGroup]);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white w-full max-w-lg rounded-lg shadow-xl border border-gray-200">
        <div className="flex items-center justify-between px-5 py-3 border-b">
          <h2 className="text-lg font-semibold">Editar Subcategoría</h2>
