...
interface CreateDivisionModalProps {
  ...
  countries: Country[]; 
  onClose: () => void;
  onSave: (data: { code: string; name: string; country_code:string; country_id:string; iso_3166_2:string; active: boolean }) => Promise<void> | void;
  saving?: boolean;
}

export const CreateDivisionModal: React.FC<CreateDivisionModalProps> = ({ open, countries = [], onClose,  onSave,   saving = false }) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white w-full max-w-lg rounded-lg shadow-xl border border-gray-200">
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          <div className="space-y-1">
            <label className="text-xs font-medium text-gray-600">ISO_3166_2</label>
            <input
              className="w-full border rounded px-3 py-2 text-sm focus:outline-none"
              value={iso_3166_2}
