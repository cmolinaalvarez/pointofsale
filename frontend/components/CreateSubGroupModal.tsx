...
interface CreateSubGroupModalProps {
  ...
  groups: Group[]; 
  onClose: () => void;
  onSave: (data: { code: string; name: string; group_id:string;  description?: string; active: boolean }) => Promise<void> | void;
  saving?: boolean;
}

export const CreateSubGroupModal: React.FC<CreateSubGroupModalProps> = ({ open, groups = [], onClose,  onSave,   saving = false }) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white w-full max-w-lg rounded-lg shadow-xl border border-gray-200">
        <div className="flex items-center justify-between px-5 py-3 border-b">
          <h2 className="text-lg font-semibold">Nuevo Subgrupo</h2>
