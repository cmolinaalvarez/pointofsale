...
interface AccountDetailModalProps { account: Account | null; onClose: () => void; }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white w-full max-w-md rounded-lg shadow-xl p-6 relative">
        ...
        <h2 className="text-xl font-semibold mb-4">Detalle de cuenta</h2>
        <div className="space-y-2 text-sm">
          <p><span className="font-medium">ID:</span> {account.id}</p>
          <p><span className="font-medium">Código:</span> {account.code}</p> 
          <p><span className="font-medium">Nombre:</span> {account.name}</p>
          <p><span className="font-medium">Descripción:</span> {account.description || '—'}</p>
          <p><span className="font-medium">Estado:</span> {account.active ? 'Activa' : 'Inactiva'}</p>
          {account.createdAt && <p><span className="font-medium">Creada:</span> {new Date(account.createdAt).toLocaleString()}</p>}
          {account.updatedAt && <p><span className="font-medium">Actualizada:</span> {new Date(account.updatedAt).toLocaleString()}</p>} 
          {account.userId && <p><span className="font-medium">Usuario actualización:</span> {account.userId}</p>}
        </div>
        <pre className="mt-4 bg-gray-100 rounded p-3 max-h-48 overflow-auto text-xs">{JSON.stringify(account, null, 2)}</pre>
...
// ✅ SOLUCIÓN: SOLO UNA INTERFACE AccountTableProps COMBINADA
          ...
          {accounts.length === 0 ? (
            <tr>
              <td colSpan={5} className="text-center py-8 text-gray-500 text-lg">
                <span role="img" aria-label="Caja vacía">📦</span> No existen accountos disponibles
              </td>
            </tr>
          ) : (
            accounts.map((account, idx) => (
              <tr key={account.id} className="hover:bg-gray-50 transition-colors">
                ...
                <td className="px-3 py-2 text-sm font-mono text-gray-900 w-32 truncate leading-tight" title={account.code}>{account.code}</td>
                <td className="px-3 py-2 text-sm font-medium text-gray-900 w-48 truncate leading-tight" title={account.name}>{account.name}</td>
                <td className="px-3 py-2 text-sm font-medium text-gray-900 w-48 truncate leading-tight" title={account.account_type}>{account.account_type}</td>
                <td className="px-2 py-2 text-sm text-center">
                  <BotonEstado
                    active={!!account.active}
                    onClick={() => onToggleActive && onToggleActive(account.id, !account.active)}
                    titleWhenActive="Desactivar cuenta"
                    titleWhenInactive="Activar cuenta"
                  />
                </td>
                <td className="px-2 py-2 text-sm font-medium flex flex-wrap gap-1 justify-center">
                  ...
                  {onEdit && (
                    <BotonAccion
                      ...
                      aria-label="Editar accounto"
                      title="Editar accounto"
