...
export function CreatePaymentTermModal({
  ...
  open: boolean;
  onClose: () => void;
  onSave: (data: { code: string; name: string; description?: string; days: number; active: boolean }) => Promise<void> | void;
  saving?: boolean;
}) {
  ...
  useEffect(()=>{ if(open){ setActive(true); setDays(0); }},[open]);
  if (!open) return null;
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    await onSave({ code: code.trim().toUpperCase(), name: name.trim(), description: description.trim() || undefined, days, active });
    setCode(""); setName(""); setDescription(""); setDays(0); setActive(true); setErr(null);
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white w-full max-w-lg rounded-lg shadow border">
        <div className="flex items-center justify-between px-5 py-3 border-b">
          <h2 className="text-lg font-semibold">Nuevo Término de Pago</h2>
        ...
        <form onSubmit={submit} className="p-5 space-y-4">
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <button type="submit" disabled={saving} className="px-4 py-2 text-sm rounded bg-green-600 text-white">{saving?'Guardando...':'Guardar'}</button>
          </div>
        </form>
      </div>
    </div>
