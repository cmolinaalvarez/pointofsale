...
interface CreateCountryModalProps {
  open: boolean;
  onClose: () => void;
  onSave: (data: { code: string; name: string; country_code?: string; active: boolean }) => Promise<void> | void;
  saving?: boolean;
}

export const CreateCountryModal: React.FC<CreateCountryModalProps> = ({ open, onClose, onSave, saving = false }) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white w-full max-w-lg rounded-lg shadow-xl border border-gray-200">
        <div className="flex items-center justify-between px-5 py-3 border-b">
          <h2 className="text-lg font-semibold">Nuevo País</h2>
          ...
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              <button
                ...
                aria-pressed={active}
                aria-label={active ? 'Desactivar país' : 'Activar país'}
