...
interface CreateSubCategoryModalProps {
  ...
  categories: Category[]; 
  onClose: () => void;
  onSave: (data: { code: string; name: string; category_id:string;  description?: string; active: boolean }) => Promise<void> | void;
  saving?: boolean;
}

export const CreateSubCategoryModal: React.FC<CreateSubCategoryModalProps> = ({ open, categories = [], onClose,  onSave,   saving = false }) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white w-full max-w-lg rounded-lg shadow-xl border border-gray-200">
        <div className="flex items-center justify-between px-5 py-3 border-b">
          <h2 className="text-lg font-semibold">Nueva Subcategoría</h2>
