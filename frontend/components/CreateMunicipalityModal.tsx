...
interface CreateMunicipalityModalProps {
  ...
  divisions: Division[]; 
  onClose: () => void;
  onSave: (data: { code: string; name: string; division_code:string; division_id:string; active: boolean }) => Promise<void> | void;
  saving?: boolean;
}

export const CreateMunicipalityModal: React.FC<CreateMunicipalityModalProps> = ({ open, divisions = [], onClose,  onSave,   saving = false }) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white w-full max-w-lg rounded-lg shadow-xl border border-gray-200">
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          <div className="space-y-1">
            <label className="text-xs font-medium text-gray-600">Código división</label>
            <input
              className="w-full border rounded px-3 py-2 text-sm focus:outline-none"
              value={division_code}
          ...
          <div className="space-y-1">
            <label className="text-xs font-medium text-gray-600">División</label>
            <select
          ...
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              <button
                ...
                aria-pressed={active}
                aria-label={active ? 'Desactivar subcategoría' : 'Activar subcategoría'}
                title={active ? 'Desactivar subcategoría' : 'Activar subcategoría'}
              >
