...
interface EditGroupModalProps {
  group: Group | null;
  subCategories: SubCategory[]; // <-- Recibe las categorías
  onClose: () => void;
  onSave: (data: { id: string; code: string; name: string; description?: string; active: boolean; subcategory_id: string }) => Promise<void> | void;
  saving?: boolean;
}
...
export const EditGroupModal: React.FC<EditGroupModalProps> = ({
  ...
  useEffect(() => {
    if (group) {
      setCode(group.code || '');
      setName(group.name || '');
      setSubCategoryId(group.subcategory_id || ''); // <-- Usar category_id
          ...
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              <button
                ...
                aria-pressed={active}
                aria-label={active ? 'Desactivar grupo' : 'Activar grupo'}
                title={active ? 'Desactivar grupo' : 'Activar grupo'}
              >
