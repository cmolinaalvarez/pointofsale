    ...
    <div className="p-6">
      ...
      <div className="bg-white shadow rounded p-4 space-y-6">
        ...
        <div className="flex items-center justify-between">
          <h3 className="font-semibold">Lista de SubGrupos ({total})</h3>
          <div className="flex gap-2 items-center">
            <span className="font-bold text-green-600 text-2xl mr-6">SubGrupos</span>
            <button
              onClick={() => refetch()}
              title="Refrescar"
        ...
        <div className="mb-4">
          <input
            type="text"
            placeholder="Buscar subgrupo por nombre..."
            value={search}
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-12">
            ...
            <span className="text-green-700 font-semibold text-lg">Cargando subgrupos...</span>
          </div>
        ) : (
          <>
            <SubGroupTable
        ...
        <ConfirmDialog
          ...
          message={pendingToggle?.value ? "¿Está seguro que desea activar esta subgrupo?" : "¿Está seguro que desea inactivar esta subgrupo?"}
          confirmText={pendingToggle?.value ? "Sí, activar" : "Sí, inactivar"}
          cancelText="Cancelar"
          onConfirm={async () => {
            if (pendingToggle) {
              const subGroup = subGroups.find(c => c.id === pendingToggle.id);
              if (subGroup) {
                const updateData: any = {
                  id: subGroup.id,
                  name: subGroup.name,
                  description: subGroup.description ?? undefined,
                  active: pendingToggle.value,
                  group_id: subGroup.group_id
                };
                if (subGroup.code) updateData.code = subGroup.code;
                await updateSubGroup(updateData);
                await refetch();
              }
            }
