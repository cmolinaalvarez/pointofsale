...
export default function AccountsPage() {
  ...
  const {
    accounts,
    accountTypeEnum, // ✅ Ahora disponible
    total,
    loading,
    error,
    refetch,
    ...
    <div className="p-6">
      ...
      <div className="bg-white shadow rounded p-4 space-y-6">
        ...
        <div className="flex items-center justify-between">
          <h3 className="font-semibold">Lista de Cuentas ({total})</h3>
          <div className="flex gap-2 items-center">
            <span className="font-bold text-green-700 text-2xl mr-6">Cuentas</span>
            ...
            {/* Botón para abrir modal de nueva accounto */}
            <button
              type="button"
        ...
        <div className="mb-4">
          <input
            type="text"
            placeholder="Buscar accounto por nombre..."
            value={search}
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-12">
            ...
            <span className="text-green-700 font-semibold text-lg">Cargando accountos...</span>
          </div>
        ) : (
          <>
            {/* Tabla de cuentas paginadas y filtradas por el backend */}
            <AccountTable
              accounts={accounts}
              loading={loading}
            ...
            <div className="flex items-center gap-4 flex-wrap text-sm">
              ...
              <div className="text-gray-500">
                {total > 0 ? (
                  ...
                  'No hay accountos para mostrar'
                )}
              </div>
            </div>
          </>
        )}
        {/* Modal de confirmación para activar/inactivar accounto */}
        <ConfirmDialog
          open={confirmOpen}
          title={pendingToggle?.value ? "Confirmar activación" : "Confirmar inactivación"}
          message={pendingToggle?.value ? "¿Está seguro que desea activar esta accounto?" : "¿Está seguro que desea inactivar esta accounto?"}
          confirmText={pendingToggle?.value ? "Sí, activar" : "Sí, inactivar"}
          cancelText="Cancelar"
          onConfirm={async () => {
            if (pendingToggle) {
              // Actualiza el estado activo/inactivo de la accounto
              const account = accounts.find(c => c.id === pendingToggle.id);
              if (account) {
              ...
              if (account.code) updateData.code = account.code;
      ...
      <EditAccountModal
        account={editing}
        accountTypeEnum={accountTypeEnum} // ✅ Pasar el enum
        saving={saving}
