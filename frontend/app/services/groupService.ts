import { GroupListResponseDTO, CreateGroupData, UpdateGroupData, GroupReadDTO } from '@/app/types/group';
...
class GroupService {
  ...
  private async makeRequest(endpoint: string, options: RequestInit = {}) {
    ...
    console.group('🚀 GroupService Request');
    console.log('📡 URL:', url);
  ...
  async getGroups(tokenOrParams?: string | {
    skip?: number;
    limit?: number; 
    search?: string;
    active?: boolean;
  ...
  async getGroup(id: string, token: string): Promise<GroupReadDTO> {
  ...
  async createGroup(subCategoryData: CreateGroupData, token: string): Promise<GroupReadDTO> {
  ...
  async updateGroup(id: string, subCategoryData: UpdateGroupData, token: string): Promise<GroupReadDTO> {
    return await this.makeRequest(`/groups/${id}`, {
      method: 'PUT',
      headers: {
        'Authorization': `Bearer ${token}`,
  ...
  async deleteGroup(id: string, token: string): Promise<any> {
    return await this.makeRequest(`/groups/${id}`, {
      method: 'DELETE',
      headers: {
        'Authorization': `Bearer ${token}`,
