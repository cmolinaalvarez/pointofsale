import { MunicipalityListResponseDTO, CreateMunicipalityData, UpdateMunicipalityData, MunicipalityReadDTO } from '@/app/types/municipality';
...
class MunicipalityService {
  ...
  private async makeRequest(endpoint: string, options: RequestInit = {}) {
    ...
    console.group('🚀 MunicipalityService Request');
    console.log('📡 URL:', url);
  ...
  async getMunicipalities(tokenOrParams?: string | {
    skip?: number;
    limit?: number; 
    search?: string;
    active?: boolean;
    ...
    console.log('🎯 getMunicipalities called');
    ...
    const endpoint = `/municipalities/${queryString ? `?${queryString}` : ''}`;
  ...
  async getMunicipality(id: string, token: string): Promise<MunicipalityReadDTO> {
  ...
  async createMunicipality(municipalityData: CreateMunicipalityData, token: string): Promise<MunicipalityReadDTO> {
  ...
  async updateMunicipality(id: string, municipalityData: UpdateMunicipalityData, token: string): Promise<MunicipalityReadDTO> {
    return await this.makeRequest(`/municipalities/${id}`, {
      method: 'PUT',
      headers: {
        'Authorization': `Bearer ${token}`,
  ...
  async deleteMunicipality(id: string, token: string): Promise<any> {
    return await this.makeRequest(`/municipalities/${id}`, {
      method: 'DELETE',
      headers: {
        'Authorization': `Bearer ${token}`,
