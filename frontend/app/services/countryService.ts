import { CountryListResponseDTO, CreateCountryData, UpdateCountryData, CountryReadDTO } from '@/app/types/country';
...
class CountryService {
  ...
  private async makeRequest(endpoint: string, options: RequestInit = {}) {
    ...
    console.group('🚀 CountryService Request');
    console.log('📡 URL:', url);
  ...
  async getCountries(tokenOrParams?: string | {
    skip?: number;
    limit?: number; 
    search?: string;
    active?: boolean;
  ...
  async getCountry(id: string, token: string): Promise<CountryReadDTO> {
    return await this.makeRequest(`/countries/${id}`, {
      method: 'GET',
      headers: {
  ...
  async createCountry(countryData: CreateCountryData, token: string): Promise<CountryReadDTO> {
  ...
  async updateCountry(id: string, countryData: UpdateCountryData, token: string): Promise<CountryReadDTO> {
    return await this.makeRequest(`/countries/${id}`, {
      method: 'PUT',
      headers: {
        'Authorization': `Bearer ${token}`,
  ...
  async deleteCountry(id: string, token: string): Promise<any> {
    return await this.makeRequest(`/countries/${id}`, {
      method: 'DELETE',
      headers: {
        'Authorization': `Bearer ${token}`,
