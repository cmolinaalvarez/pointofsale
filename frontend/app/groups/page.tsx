...
export default function GroupsPage() {
  ...
  const { subCategories, loading: loadingSubCategories } = useSubCategories(token);
    ...
    <div className="p-6">
      ...
      <div className="bg-white shadow rounded p-4 space-y-6">
        ...
        <div className="flex items-center justify-between">
          <h3 className="font-semibold">Lista de Grupos ({total})</h3>
          <div className="flex gap-2 items-center">
            <span className="font-bold text-green-600 text-2xl mr-6">Grupos</span>
            <button
              onClick={() => refetch()}
              title="Refrescar"
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-12">
            ...
            <span className="text-green-700 font-semibold text-lg">Cargando grupos...</span>
          </div>
        ) : (
          <>
            <GroupTable
        ...
        <ConfirmDialog
          ...
          message={pendingToggle?.value ? "¿Está seguro que desea activar esta grupo?" : "¿Está seguro que desea inactivar esta grupo?"}
          confirmText={pendingToggle?.value ? "Sí, activar" : "Sí, inactivar"}
          cancelText="Cancelar"
          onConfirm={async () => {
            if (pendingToggle) {
              const group = groups.find(c => c.id === pendingToggle.id);
              if (group) {
