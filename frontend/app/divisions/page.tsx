...
export default function DivisionsPage() {
  ...
  const { countries, loading: loadingCountries } = useCountries(token);
  ...
  // Debug: revisar datos de divisiones cuando cambien
  useEffect(() => {
    if (divisions.length > 0) {
      console.log("🏢 División de ejemplo:", divisions[0]);
      console.log("🏢 Country name:", divisions[0].country_name);
      console.log("🏢 Country ID:", divisions[0].country_id);
    }
  }, [divisions]);
    ...
    <div className="p-6">
      ...
      <div className="bg-white shadow rounded p-4 space-y-6">
        ...
        <div className="flex items-center justify-between">
          <h3 className="font-semibold">Lista de Divisiones ({total})</h3>
          <div className="flex gap-2 items-center">
            <span className="font-bold text-green-600 text-2xl mr-6">Divisiones</span>
            <button
              onClick={() => refetch()}
              title="Refrescar"
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-12">
            ...
            <span className="text-green-700 font-semibold text-lg">Cargando divisiones...</span>
          </div>
        ) : (
          <>
            <DivisionTable
        ...
        <ConfirmDialog
          ...
          onConfirm={async () => {
            if (pendingToggle) {
              const division = divisions.find(d => d.id === pendingToggle.id);
              if (division) {
                const updateData: any = {
                  ...
                  iso_3166_2: division.iso_3166_2,
                  country_id: division.country_id  // Corregido: era category_id
                };
                if (division.code) updateData.code = division.code;
                console.log("🔄 Actualizando división:", updateData);
                try {
                  await updateDivision(updateData);
                  await refetch();
